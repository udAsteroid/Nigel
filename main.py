...
@bot.event
async def on_ready():
    print(f'We have logged in as {bot.user}! Current time is {datetime.datetime.now(pytz.timezone("Australia/Melbourne")).hour}!')
...
async def looptask():
    ...
    if datetime.datetime.now(pytz.timezone("Australia/Melbourne")).hour >= 6:
        ...
        if not moonperms.read_messages:
            ...
            await moonchannel.send(embed=embed, files=[nextcord.File('go to sleep.mp4')])
...
async def ping(interaction: nextcord.Interaction):
    await interaction.send("Pong!", files=[nextcord.File('go to sleep.mp4')])
